//! Interface factory
//!
//! This is the easiest way to create a driver instance. You can set various
//! parameters of the driver and give it an interface to use. The builder will
//! return a [`mode::RawMode`](../mode/raw/struct.RawMode.html) object which you
//! should coerce to a richer display mode, like
//! [mode::Graphics](../mode/graphics/struct.GraphicsMode.html) for drawing
//! primitives and text.
//!
//! # Examples
//!
...
//! tree in user crate.
//!
//! The above example will produce a [RawMode](../mode/raw/struct.RawMode.html)
//! instance by default. You need to coerce them into a mode by specifying a
//! type on assignment. For example, to use [`GraphicsMode`
//! mode](../mode/graphics/struct.GraphicsMode.html):
//!
//! ```rust,no_run
//! use oled_async::{mode::GraphicsMode, Builder};
...
where
    DV: DisplayVariant,
{
    /// Create new builder with a default size of 128 x 64 pixels and no
    /// rotation.
...
where
    DV: DisplayVariant,
{
    /// Set the rotation of the display to one of four values. Defaults to no
    /// rotation.
    pub fn with_rotation(self, rotation: DisplayRotation) -> Self {
        Self { rotation, ..self }
    }

    /// Finish the builder and use the given interface to communicate with the
    /// display.
    ...
    {
        let properties =
            DisplayProperties::new(self.variant, interface, self.rotation);
